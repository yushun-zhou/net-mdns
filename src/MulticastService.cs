    ...
    : IMulticastService
{
    ...
    public void SendAnswer(Message answer, MessageEventArgs query, bool checkDuplicate = true)
    {
        ...
        foreach (var r in answer.Answers) r.TTL = r.TTL > maxLegacyUnicastTTL ? maxLegacyUnicastTTL : r.TTL;
        foreach (var r in answer.AdditionalRecords) r.TTL = r.TTL > maxLegacyUnicastTTL ? maxLegacyUnicastTTL : r.TTL;
        foreach (var r in answer.AdditionalRecords) r.TTL = r.TTL > maxLegacyUnicastTTL ? maxLegacyUnicastTTL : r.TTL;
    ...
    public void OnDnsMessage(object? sender, UdpReceiveResult result)
    {
        ...
        if (msg.Opcode != MessageOperation.Query || msg.Status != MessageStatus.NoError) return;
        ...
        try
        {
            if (msg.IsQuery && msg.Questions.Count > 0)
                QueryReceived?.Invoke(this,
                    new MessageEventArgs { Message = msg, RemoteEndPoint = result.RemoteEndPoint });
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description>interface is enabled</description>
    ///         </item>
    ///         <item>
    ///             <description>interface is not a loopback</description>
    ///         </item>
    ///     </list>
    ///     <para>
    ...
    internal async void Send(Message msg, bool checkDuplicate, IPEndPoint? remoteEndPoint = null)
    {
        ...
        if (checkDuplicate && !sentMessages.TryAdd(packet)) return;
